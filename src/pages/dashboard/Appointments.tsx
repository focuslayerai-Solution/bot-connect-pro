...
import { useAppointments, useAppointmentStats, useUpdateAppointmentStatus } from "@/hooks/useAppointments";
...
export default function Appointments() {
  ...
  const filteredAppointments = appointments.filter(
    (apt) =>
      ...
      apt.customer_number.includes(searchQuery) ||
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{apt.service || "General"}</p>
                    </div>
                  </div>
            ...
            <Table>
              ...
              <TableBody>
                {filteredAppointments.map((apt) => (
                  <TableRow key={apt.id}>
                    <TableCell className="font-medium">{apt.id.slice(0, 8)}</TableCell>
                    <TableCell>
                      <div>
                        <p className="font-medium">{apt.customer_name || "Unknown"}</p>
                        <p className="text-sm text-muted-foreground">{apt.customer_number}</p>
                      </div>
                    </TableCell>
                    ...
                    <TableCell className="hidden md:table-cell">{apt.service || "General"}</TableCell>
                    <TableCell>
                      <StatusBadge variant={getStatusVariant(apt.status)}>
                        {getStatusLabel(apt.status)}
                      </StatusBadge>
